  ...
  for (byte s = 0; s < 5; s++)
  {
    if (array_BLA[s] == pack_id)
    {
      ...
      if (pack_id == PDR_ID)
      {
        ...
        byte wait_id = BLA_unpacker.waiting_pack_id(arr);
...
//      for (byte r = 0; r < 35; r++)
//      {
//        packet_sender[r] = big_array_NKR[f][r];
  ...
  if (f < 5)
  {
    ...
    switch (action)
    {
      case (CTT_ID):          //"Взлет разрешен"
        {
          ...
          while (!BLA_take_id(PDR_ID, CTT_ID))
          {
            //Отправляем пакет на БЛА
            BLA_sender.send_to_BLA(packet_sender);
          ...
          //Ожидаем окончания взлета
          while (!BLA_take_id(TCM_ID))
          {}
      ...
      case (RTM_ID):          //"Начало движения"
        {
          ...
          while (!BLA_take_id(PDR_ID, RTM_ID))
          {
            //Отправляем пакет на БЛА
            BLA_sender.send_to_BLA(packet_sender);
          }
      ...
      case (MCM_ID):          //"Осмотр завершен"
        {
          crc_pack = packet_sender[18];
          while (!BLA_take_id(PDR_ID, MCM_ID))
          {
            //Отправляем пакет на БЛА
            BLA_sender.send_to_BLA(packet_sender);
      ...
      case (CTL_ID):          //"Посадка разрешена"
        {
          ...
          while (!BLA_take_id(PDR_ID, CTL_ID))
          {
            //Отправляем пакет на БЛА
            BLA_sender.send_to_BLA(packet_sender);
          }
      ...
      case (TPM_ID):          //"Параметры траектории"
        {
          ...
          if (count_tpm_num == 0)
          {
            for (byte d = 0; d < 35; d++)
            {
              ...
              if ((d > 19) & (d < 32))
              {
                orient_tpm[tpm_ori_count] = packet_sender[d];
              ...
              {
                coord_tpm[tpm_coord_count] = packet_sender[d];
                //                SERIAL_TM.print("id byte = " + String(d)); SERIAL_TM.print("\t");
              ...
              else
              {
                ...
                tpm_pack_BLA[tpm_count] = packet_sender[d];
                //                SERIAL_TM.print("id byte = " + String(d)); SERIAL_TM.print("\t");
